...
<template>
	<section class="min-h-48 w-full h-full p-0 flex-col">
		<div class="relative w-full h-full">
			<div :id="'carousel-' + name" class="carousel carousel-center overflow-y-hidden overflow-x-auto w-full h-full max-h-full">
				<slot name="items"></slot>
			</div>

			<!-- Navigation -->
			<div v-if="!hideNavigation" class="absolute left-0 right-0 top-2/5 flex origin-center transform justify-between">
				<button :id="'carousel-btn-prev-' + name" @click="prevClick" class="absolute top-1/2 left-5 btn btn-ghost btn-primary p-2">
				...
				<button :id="'carousel-btn-next-' + name" @click="nextClick" class="absolute top-1/2 right-5 btn btn-ghost btn-primary p-2">
