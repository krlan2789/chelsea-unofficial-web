...
const { data: fixtures } = useTeamFixtures();

const teamFixtures = ref<IFixtureCardProp[] | null>(data ?? fixtures ?? []);
const showCount = ref<number>(limit ?? 18);

const visibleFixtures = computed(() =>
	teamFixtures.value?.slice(0, showCount.value) ?? []
);
</script>

<template>
	<ClientOnly v-if="visibleFixtures" fallback-tag="section">
		<CarouselComponent name="fixtures">
			<template #items>
				<FixtureCardComponent v-for="(fixture, index) in visibleFixtures" :key="index"
					:id="'' + fixture.fixtureId" class="carousel-item relative w-screen max-w-[480px]" :fixture />
			</template>
		</CarouselComponent>
	</ClientOnly>
