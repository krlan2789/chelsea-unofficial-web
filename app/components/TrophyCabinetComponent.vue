...
onMounted(async () => {
	if (!data) {
		const { data: trophies } = await useTrophyCabinet();
		if (trophies) {
...
<template>
	<ClientOnly v-if="trophyListByYear" fallback-tag="section">
		...
		<section class="mt-16 px-4 xl:px-0">
			...
			<ul class="timeline timeline-snap-icon max-md:timeline-compact timeline-vertical flex-col-reverse mt-8 px-8 md:px-0">
				<li v-for="([year, items], index) in Object.entries(trophyListByYear)" :key="index" class="border-0 bg-transparent">
