...
const newsList = ref<INewsCardProp[]>(data ?? news ?? []);
const showCount = ref<number>(limit ?? 18);

const visibleNews = computed(() =>
	newsList.value?.slice(0, showCount.value) ?? []
);
...
<template>
	<ClientOnly v-if="visibleNews.length" fallback-tag="section">
		...
		<div class="flex flex-col w-full h-auto px-4 xl:px-0 pb-8">
			<div class="divider divider-primary">
				<button v-if="lazy && showCount < newsList.length" @click="loadMore" class="btn btn-primary uppercase">
					Load more
				</button>
